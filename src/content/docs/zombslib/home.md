---
title: Home
description: Official zombslib docs
sidebar:
    order: 0
---

This is the official documentation for [zombslib](https://github.com/creaffy/zombslib).

## Credits

Zombslib was inspired by [Zombs.io Client Library](https://github.com/particle99/zombs-client-library-ts). Make sure to check it out if you're interested.<br/>
The rest of the credit goes to us, the group of reverse-engineers behind figuring out the protocol and making it all work - xor&pray :)

## Purpose

This project is intended to lay the foundation for all future ZR network-based projects. It is supposed to be a minimal library focused on delivering easy-to-use packet decoders and encoders reimplementations, along with a comprehensive API for controlling bots in-game out of the box.

## Contact

Join our [Discord server](https://discord.gg/E5QWPx6TrX) if you need any help or are interested in contributing!
